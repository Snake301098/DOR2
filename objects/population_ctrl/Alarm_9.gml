///@description spawn ships CP room

var a,b,xx,yy,ship_numb, numb_waiters, numb_corp, numb_travelers,numb_bonus_grubers,numb_hunters_mob, totalships, mmoships, eicships, vruships;
...
var totalships, mmoships, eicships, vruships;

var cp_spawn_ship_delay = irandom_range(10,30) * room_speed;
...
// ****LvX_1-----------------        
if (room=lv1_1 or room=lv2_1 or room=lv3_1) then //check map level n: X_n Y_n Z_n
    {
    if room = global.cproom then //IF CP ROOM
    {
		cproom_spawn_ships();
        alarm[9]=cp_spawn_ship_delay;
    }
    else 
    { //NOT CP ROOM
        alarm[9]=room_speed*irandom_range(20,40);  
    } 
}
...
if (room=lv1_2 or room=lv2_2 or room=lv3_2) then
    ...
    for (i=1; i<=3-numb_bonus_grubers; i+=1)
            ...
            with(instance_create_depth(a.x,a.y,ship))
                ...
                alarm[5]=irandom(500);
                action[0]:="bonus_grub";
                corporation:=info_map(room,"owners");
...
if (room=lv1_4 or room=lv2_4 or room=lv3_4) then
    ...
    if instance_number(ships)<ship_numb*2 then
        ...
        with(teleport)
            ...
            if map1=room then a:=map2 else a:=map1;
                if info_map(a,"owners")<=2 then
                    {
                    if info_map(a,"owners")=info_map(room,"owners") then
                        while numb_corp[info_map(a,"owners")]<(ship_numb-irandom(3)) do
                            {
                            with(instance_create_depth(x,y,ship))
                                {
                                corporation=info_map(a,"owners");
                    ...
                    else while numb_corp[info_map(a,"owners")]<((ship_numb div 2)-irandom(4))div 2 do
                            {
                            with(instance_create_depth(x,y,ship))
                                {
                                corporation=info_map(a,"owners");
                ...
                else while numb_corp[info_map(room,"owners")]<ship_numb div 2 do
                            {
                            with(instance_create_depth(x,y,ship))
                                {
                                corporation=info_map(room,"owners");
    ...
    for (i=1; i<=choose(1,2)-numb_travelers; i+=1)
            ...
            with(teleport) 
                {
                if info_map(map1,"owners")=info_map(room,"owners") and
                    info_map(map2,"owners")=info_map(room,"owners") then
                        {
                        a:=id;
                        }
                else if info_map(map1,"owners")=3 or
                    info_map(map2,"owners")=3 then
                        {
                        b:=id;
                        }
                }
...
//if room = global.cproom then alarm[9]=room_speed*15 else alarm[9]=room_speed*5;