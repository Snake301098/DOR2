...
if own_health_bilo>own_health then 
    {
    if own_health<=health_def*0.10 then {runing=true; alarm[1]=1;}
    //own_health_bilo=own_health;
    //own_shield_bilo=own_shield;
    restore=false;
    alarm[3]=-1;
    }
else  
    {
    if alarm[3]<0 and restore=0 then 
        {
        alarm[3]=300*2;
        restore=2;
        }
...
var i;
//Äåêòèâàöèÿ
if distance_to_object(gamer)>1500 then
    {
    if DroidOff=false then
        {
        for (i:=1; i<=12; i+=1)
            instance_deactivate_object(droid[i,0]);
        DroidOff=true;
        }
    }
//Àêòèâàöèÿ
else if DroidOff=true then
    {
    for (i:=1; i<=12; i+=1)
        {
