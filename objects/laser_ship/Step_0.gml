...
if owner.id = gamer.id then
{
    if targetedship != 0 then
        ...
        direction=point_direction(x,y,targetedship.x,targetedship.y);
        image_angle = set_angle(image_angle, direction, 10); //10
        if point_distance(x,y,targetedship.x,targetedship.y)<5 or point_distance(x,y,gamer.x,gamer.y)>550 then {instance_destroy();}
//        if point_distance(x,y,targetedship.x,targetedship.y)<5 or point_distance(x,y,gamer.x,gamer.y)>330 then {instance_destroy();} //or point_distance(x,y,xowner,yowner)>650
    } else {
		...
		//final_targetx=lengthdir_x(400+abs(+gamer.x-xowner),point_direction(xowner,yowner,targetx,targety))
		//final_targety=lengthdir_y(400+abs(+gamer.y-yowner),point_direction(xowner,yowner,targetx,targety))
        ...
        image_angle = set_angle(image_angle, direction, 10); //10
        //if point_distance(x,y,gamer.x,gamer.y)>330 then {instance_destroy();}
        if point_distance(x,y,targetx,targety)<30 or point_distance(x,y,gamer.x,gamer.y)>550 then {instance_destroy();}
    }
} 
else
{
    if instance_exists(target) then
    {
        ...
        direction=point_direction(x,y,target.x,target.y);
        image_angle = set_angle(image_angle, direction, 10); //10
//        if point_distance(x,y,target.x,target.y)<5 or point_distance(x,y,owner.x,owner.y)>330 then {instance_destroy();}
...
decrease_angle_factor = decrease_angle_factor * 0.992;
